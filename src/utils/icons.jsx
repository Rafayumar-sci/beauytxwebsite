...
export function generateServiceLongDescription(service, category) {
  ...
  return `${service.description} At Beautyx by Farina, our ${service.name.toLowerCase()} service is performed by experienced professionals using premium products and the latest techniques. Located on Main Boulevard, Lahore, we are a trusted destination for ${category.name.toLowerCase()} services. ${kwList ? `Clients searching for ${kwList} trust our expertise and personalized approach.` : ""} We tailor every ${service.name.toLowerCase()} session to your specific needs, ensuring results that exceed expectations. Book your appointment today and experience the Beautyx difference.`;
}
